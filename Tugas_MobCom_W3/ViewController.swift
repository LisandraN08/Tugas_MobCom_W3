...
class ViewController: UIViewController {
    ...
    @IBAction func numbersButtons(_ sender: UIButton) {
        if resultLabel.text!.count < 15 {
                   if sender.tag == 0 {
                       if number.first == "0" && (number.count == 1 || number[number.index(number.startIndex, offsetBy: 1)] != ".") {
                           number.removeFirst()
                       }
                       number += "0"
            ...
            let alert = UIAlertController(title: "Error", message: "You entered too large number!", preferredStyle: .alert)
    ...
    @IBAction func deleteLastNumber(_ sender: UIButton) {
        if number != "" {
            ...
            let alert = UIAlertController(title: "Error", message: "Please enter digits!", preferredStyle: .alert)
    ...
    @IBAction func percentButton(_ sender: UIButton) {
        if !number.isEmpty {
            ...
            let alert = UIAlertController(title: "Error", message: "Enter the percent sign after the number!", preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .default)
            alert.addAction(action)
            self.present(alert, animated: true)
        }
    ...
    @IBAction func operations(_ sender: UIButton) {
        if sender.tag == 10 {
            ...
            if validInput() && validInputForMinus() && validInputForPercent() {
                ...
                let alert = UIAlertController(title: "Error", message: "Incorrect characters entered", preferredStyle: .alert)
                let action = UIAlertAction(title: "OK", style: .default) { _ in
                    self.number = ""
                    self.resultLabel.text = "0"
                }
